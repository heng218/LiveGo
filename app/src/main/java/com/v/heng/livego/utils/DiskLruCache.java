...
/**
 * A simple disk LRU bitmap cache to illustrate how a disk cache would be used for bitmap caching. A much more robust and efficient disk LRU cache solution can
 * be found in the ICS source code (libcore/luni/src/main/java/libcore/io/DiskLruCache.java) and is preferable to this simple implementation.
 */
public class DiskLruCache {
	...
	/**
	 * Constructor that should not be called directly, instead use {@link DiskLruCache#openCache(Context, File, long)} which runs some extra checks before
	...
	/**
	 * Flush the cache, removing oldest entries if the total size is over the specified cache size. Note that this isn't keeping track of stale files in the
	 * cache directory that aren't in the HashMap. If the images and keys in the disk cache change often then they probably won't ever be removed.
	 */
